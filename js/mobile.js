 ...
 *
 * FIXES:
 * 1. Seletor de cores corrigido: '.quick-color' → '.qcolor' (classe real no HTML).
 * 2. Semântica clear corrigida:
 *    - Celular envia 'clear:mobile' (não 'clear:local') ao limpar os próprios traços.
 *    - onMsg: 'clear:pc' limpa remDone (traços do PC); 'clear:mobile' limpa localStrokes.
 *    - O original tinha exatamente o oposto — cada mensagem apagava o lado errado.
 * 3. drawLayer: removido ternário inútil `isRemote ? pdfPage : pdfPage`.
 *
...
const cvs      = $('draw-canvas');
const cvsBG    = $('bg-canvas');
...
const localStrokes = new Map(); // Map<page, stroke[]>  — traços do celular
let   activeStroke = null;

const remActive = {};           // id → stroke em progresso (do PC)
const remDone   = new Map();    // Map<page, stroke[]>  — traços concluídos do PC

// ── Gesture state ───────────────────────────────────────────────────────────────
let gesture  = 'idle'; // 'idle' | 'drawing' | 'panning' | 'pinching'
const ptrs   = new Map();
let panLX = 0, panLY = 0;
...
;(function restorePrefs() {
  ...
  // FIX #1: seletor correto (.qcolor, não .quick-color)
  qsa('.qcolor').forEach(b => b.classList.toggle('active', b.dataset.color === color));
...
function redraw() {
  drawBG();
  drawGrid();
  drawLayer(cvsRem,  remDone,      /* isRemote = */ true);
  drawLayer(cvs,     localStrokes, /* isRemote = */ false);
  drawActiveStroke();
  drawRemActive();
}
...
function drawBG() {
  ...
  if      (bgMode === 'dark')   ctx.fillStyle = '#1a1a2e';
  else if (bgMode === 'light')  ctx.fillStyle = '#f8f8f5';
  else if (bgMode === 'amoled') return;
  else if (bgMode === 'grid')   ctx.fillStyle = '#0d0d0f';
  else                          ctx.fillStyle = '#1a1a2e';

  if (bgMode !== 'amoled') ctx.fillRect(0, 0, csW * dpr, csH * dpr);
...
/** FIX: removido ternário sem sentido `isRemote ? pdfPage : pdfPage`. */
function drawLayer(canvas, strokeMap, isRemote) {
  const ctx = canvas?.getContext('2d'); if (!ctx) return;
  ctx.clearRect(0, 0, csW * dpr, csH * dpr);
  const strokes = strokeMap.get(pdfPage) ?? [];
  ctx.save();
  const cx = Math.max(0, -vp.x * vp.z * dpr);
  const cy = Math.max(0, -vp.y * vp.z * dpr);
  const cw = Math.min(csW * dpr, vp.docW * vp.z * dpr);
  const ch = Math.min(csH * dpr, vp.docH * vp.z * dpr);
...
function drawRemActive() {
  ...
  const entries = Object.values(remActive); if (!entries.length) return;
  ctx.save();
  const cx = Math.max(0, -vp.x * vp.z * dpr);
  const cy = Math.max(0, -vp.y * vp.z * dpr);
  const cw = Math.min(csW * dpr, vp.docW * vp.z * dpr);
...
function drawStroke(ctx, s) {
  ...
  const t   = TOOLS_MOB[s.tool] ?? TOOLS_MOB.pen;
  ctx.save();
  ...
  if (pts.length === 1) {
    ...
    const a = sc(pts[0].nx, pts[0].ny);
    const b = sc(pts[pts.length-1].nx, pts[pts.length-1].ny);
    ...
    if (pts.length === 2) {
      const p1 = sc(pts[1].nx, pts[1].ny); ctx.lineTo(p1.x, p1.y);
    } else {
      for (let i = 1; i < pts.length - 1; i++) {
        const c = sc(pts[i].nx,   pts[i].ny);
        const n = sc(pts[i+1].nx, pts[i+1].ny);
...
function drawSegIncr(ctx, s, prevPt, newPt) {
  ...
  if (!prevPt) {
    ...
    const a = vp.normToScreen(prevPt.nx, prevPt.ny);
    const b = vp.normToScreen(newPt.nx, newPt.ny);
...
function onDown(e) {
  ...
  if (ptrs.size === 1 && gesture === 'idle') {
    ...
    if (mode === 'draw') { gesture = 'drawing'; beginStroke(x, y); }
    else                 { gesture = 'panning'; panLX = e.clientX; panLY = e.clientY; }
  } else if (ptrs.size === 2) {
...
function onMove(e) {
  e.preventDefault();
  ptrs.set(e.pointerId, e);
  if      (gesture === 'drawing' && e.pointerId === firstId()) { const { x, y } = canvasXY(e); continueStroke(x, y); }
  ...
  } else if (gesture === 'pinching' && ptrs.size >= 2) { doPinch(); }
}
...
function onUp(e) {
  ...
  if      (gesture === 'drawing' && ptrs.size === 0) { finishStroke(); gesture = 'idle'; }
    ...
    if      (ptrs.size === 1) { gesture = 'panning'; const rem = [...ptrs.values()][0]; panLX = rem.clientX; panLY = rem.clientY; }
...
function doPinch() {
  ...
  if (!zoomLocked && pinchDist > 0) vp.zoomAt(vp.z * (dist / pinchDist), fx, fy);
  const sens = parseFloat($('pan-sens')?.value ?? '1');
...
function finishStroke() {
  activeStroke = null;
  peer?.send({ type: 'stroke:end', id: curStrokeId });
  curStrokeId  = null;
}
...
function cancelStroke() {
  if (activeStroke) {
    ...
    const idx = arr.lastIndexOf(activeStroke);
...
function onMsg(msg) {
  ...
  switch (msg.type) {
    ...
    case 'stroke:start': {
      ...
      remActive[msg.id] = s;
      const ctx = cvsRem?.getContext('2d');
      if (ctx) drawSegIncr(ctx, s, null, { nx: msg.nx, ny: msg.ny });
      break;
    }
    ...
    case 'stroke:move': {
      ...
      const ctx = cvsRem?.getContext('2d');
      if (ctx) drawSegIncr(ctx, s, prev, { nx: msg.nx, ny: msg.ny });
      break;
    }

    case 'stroke:end': {
      const s = remActive[msg.id];
      if (s) {
        remDone.has(pdfPage) || remDone.set(pdfPage, []);
        remDone.get(pdfPage).push(s);
      }
      delete remActive[msg.id];
      break;
    }
    ...
    case 'clear:all':
      localStrokes.clear();
      remDone.clear();
      Object.keys(remActive).forEach(k => delete remActive[k]);
      markDirty();
      break;

    // FIX: semântica corrigida.
    // 'clear:pc' = PC limpou traços do PC → no celular, são os traços "remotos" (remDone)
    case 'clear:pc':
      if (msg.page != null) {
        const arr = remDone.get(msg.page); if (arr) arr.splice(0);
      } else {
        remDone.clear();
        Object.keys(remActive).forEach(k => delete remActive[k]);
      }
    ...
    // 'clear:mobile' = PC limpou traços do celular → no celular, são os traços locais
    case 'clear:mobile':
      if (msg.page != null) {
        localStrokes.set(msg.page, []);
      } else {
        localStrokes.clear();
      }
      markDirty();
    ...
    case 'undo': {
      const arr = remDone.get(pdfPage) ?? [];
      if (arr.length) { arr.pop(); markDirty(); }
      break;
    }
    ...
    case 'pdf:thumb': {
      const img = new Image();
      img.onload = () => { pdfThumb = img; markDirty(); };
      img.src    = msg.data;
      break;
    }
...
// Auto-preenchimento via URL (?code=XXXXXX)
;(() => {
...
// FIX: seletor corrigido — '.qcolor' (não '.quick-color')
qsa('.qcolor').forEach(b => b.addEventListener('click', () => {
  qsa('.qcolor').forEach(x => x.classList.remove('active'));
  b.classList.add('active');
  color = b.dataset.color; setPref('color', color);
  const mc = $('mob-color'); if (mc) mc.value = color;
}));

on('mob-color', 'input', e => {
  color = e.target.value; setPref('color', color);
  qsa('.qcolor').forEach(x => x.classList.remove('active'));
});

on('mob-size', 'input', e => {
  size = parseInt(e.target.value); setPref('size', size); updateSizeDot(size);
});

function updateSizeDot(v) {
  const d = $('size-dot');
  if (d) { const px = Math.min(40, Math.max(2, v)); d.style.width = px + 'px'; d.style.height = px + 'px'; }
...
// FIX: envia 'clear:mobile' com página — perspectiva independente
on('btn-mob-clear', 'click', () => {
  ...
  localStrokes.set(pdfPage, []);
  markDirty();
  peer?.send({ type: 'clear:mobile', page: pdfPage });
});
