 ...
 *
 * FIX: Canal agora usa ordered:true (confiável).
 * O original usava ordered:false, maxRetransmits:0 (UDP-like),
 * causando perda e reordenação de mensagens de stroke.
 */
...
export class Peer {
  ...
  async startHost() {
    ...
    // FIX: ordered:true garante entrega confiável e em ordem
    this.ch = this.pc.createDataChannel('dc', { ordered: true });
  ...
  /** Envia um objeto ou string pelo DataChannel. Retorna false se canal fechado. */
  send(data) {
    if (this.ch?.readyState !== 'open') return false;
    try {
      this.ch.send(typeof data === 'string' ? data : JSON.stringify(data));
      return true;
    } catch (e) {
      console.warn('[WebRTC] send falhou:', e.message);
      return false;
    }
  }
  ...
  _setupCh(ch) {
    ch.onopen    = () => { console.log('[WebRTC] canal aberto'); this._setState(CS.CONNECTED); };
    ch.onclose   = () => this._setState(CS.DISCONNECTED);
    ch.onerror   = () => this._setState(CS.ERROR);
    ch.onmessage = ({ data }) => {
      try   { this.onMsg(JSON.parse(data)); }
      catch { this.onMsg(data); }
    };
  }
